...
import {
   ...
   FileSpreadsheet, Download, Printer, Table, LayoutList, Landmark
} from 'lucide-react';
...
const Reports: React.FC<Props> = ({ employees, lang, user }) => {
   ...
   const t = {
      fr: { title: "Reporting & Livre de Paie", finance: "Vue d'ensemble", audit: "Audit IA", ledger: "Livre de Paie", btnReport: "RAPPORT INSPECTION", mass: "Masse Salariale", compliance: "Conformité" },
      ar: { title: "التقارير ودفتر الأجور", finance: "نظرة عامة", audit: "تدقيق الذكاء", ledger: "دفتر الأجور", btnReport: "تقرير التفتيش", mass: "كتلة الأجور", compliance: "الامتثال" }
   }[lang === 'ar' ? 'ar' : 'fr'];
         ...
         <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-print">
            <div className="space-y-1">
               ...
               <div className="flex bg-white p-1 rounded-xl border border-[#E5E7EB] mt-4 shadow-sm w-fit">
                  <button onClick={() => setActiveTab('finance')} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'finance' ? 'bg-[#F0F7FF] text-[#0078D4]' : 'text-[#64748B]'}`}>{t.finance}</button>
                  <button onClick={() => setActiveTab('ledger')} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'ledger' ? 'bg-[#F0F7FF] text-[#0078D4]' : 'text-[#64748B]'}`}>{t.ledger}</button>
                  <button onClick={() => setActiveTab('audit')} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'audit' ? 'bg-[#F0F7FF] text-[#0078D4]' : 'text-[#64748B]'}`}>{t.audit}</button>
               </div>
            </div>
            <div className="flex gap-3">
               <button onClick={() => alert('Génération de l\'Etat 9421 (IR) en cours...')} className="bg-[#0A9E9A] text-white px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3 shadow-xl hover:brightness-110 transition-all">
                  <Landmark size={18} /> ETAT 9421 (IR)
               </button>
               ...
               <div className="lg:col-span-8 airbnb-card p-10 bg-white shadow-sm border-[#E5E7EB]">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                     <div className="p-8 bg-[#F0FFF4] rounded-[32px] border border-[#DCFCE7] flex flex-col justify-center min-h-[200px]">
                        <p className="text-[10px] font-black text-[#64748B] uppercase tracking-widest mb-4">{t.mass}</p>
                     ...
                     <div className="p-8 bg-[#F0F7FF] rounded-[32px] border border-[#DBEAFE] flex flex-col justify-center min-h-[200px]">
                        <p className="text-[10px] font-black text-[#64748B] uppercase tracking-widest mb-4">Cotisations Est.</p>
            ...
            <div className="airbnb-card bg-white border-[#E5E7EB] shadow-sm overflow-hidden">
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-[10px] border-collapse min-w-[1200px]">
                     <thead>
                        <tr className="bg-gray-100 border-b border-gray-200 text-[#334155] font-black uppercase">
                              ...
                              <tr key={emp.id} className="hover:bg-gray-50 transition-all group">
                                 <td className="p-4 font-black border-r whitespace-nowrap">
                                    <div className="flex flex-col">
                                       ...
                                       <span className="text-[8px] text-[#64748B] font-mono">CIN: {emp.cin}</span>
                                    </div>
                                 </td>
                                 <td className="p-4 text-right border-r font-medium">{pay.baseSalary.toLocaleString()}</td>
                                 <td className="p-4 text-right border-r font-medium text-[#64748B]">{pay.seniorityBonus.toLocaleString()}</td>
               ...
               {auditData.map(record => (
                  <div key={record.employee_id} className="airbnb-card p-6 bg-white border-[#E5E7EB] flex items-center justify-between">
                     <div className="text-start">
                        ...
                        <p className="text-[10px] text-[#64748B] font-bold uppercase">{record.jobTitle}</p>
                     </div>
