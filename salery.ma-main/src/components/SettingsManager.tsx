...
import { Language, AuthUser, SystemUser, CompanyProfile, UserRole } from '../types';
import { MONTHLY_SMIG, CNSS_CEILING, CNSS_SALARIAL_RATE, AMO_SALARIAL_RATE, PROF_EXPENSES_CAP } from '../constants';
...
const SettingsManager: React.FC<Props> = ({ lang, user, setUser }) => {
  const [activeSubTab, setActiveSubTab] = useState<'branding' | 'users' | 'fiscality' | 'security'>('branding');
  const [showUserModal, setShowUserModal] = useState(false);
  const [editingUserId, setEditingUserId] = useState<string | null>(null);

  // Default to security tab for employees
  useEffect(() => {
  ...
  const t = {
    fr: {
      ...
      securityTitle: "Sécurité & Accès", securitySub: "Gérez vos identifiants et la sécurité de votre compte personnel.",
    ...
    ar: {
      ...
      securityTitle: "الأمان والولوج", securitySub: "إدارة بيانات الاعتماد وأمان حسابك الشخصي.",
      addUser: "إضافة مدير",
  ...
  const allowedRoles: UserRole[] = ['DIRECTEUR_RH', 'TEAM_RH', 'COMPTABLE', 'SITE_MANAGER', 'EMPLOYEE'];
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="text-start">
          <h2 className="text-3xl font-black text-[#0F172A] tracking-tighter">
            {user.role === 'EMPLOYEE' ? t.securityTitle : 'Configuration'}
          </h2>
          <p className="text-[#64748B] mt-1 font-medium italic">
            {user.role === 'EMPLOYEE' ? t.securitySub : 'Paramètres globaux Salery.ma • V26 Sovereign Infrastructure'}
          </p>
        </div>
        ...
        {user.role !== 'EMPLOYEE' && (
          <div className="flex bg-[#F1F5F9] p-1.5 rounded-2xl border border-[#E2E8F0] shadow-sm overflow-x-auto no-scrollbar max-w-full">
            ...
            <button onClick={() => setActiveSubTab('users')} className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${activeSubTab === 'users' ? 'bg-white shadow-md text-teal-600' : 'text-gray-400'}`}>{t.users}</button>
            <button onClick={() => setActiveSubTab('fiscality')} className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${activeSubTab === 'fiscality' ? 'bg-white shadow-md text-teal-600' : 'text-gray-400'}`}>{t.fiscality}</button>
          </div>
        )}
      </header>

      {user.role === 'EMPLOYEE' || activeSubTab === 'security' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
           <section className="bg-white rounded-[32px] p-10 border border-[#E5E7EB] shadow-sm text-start space-y-8">
              <h3 className="text-xl font-black tracking-tight flex items-center gap-3">
                 <Lock className="text-[#0078D4]" size={24} />
                 SÉCURITÉ DU COMPTE
              </h3>
              <div className="space-y-6">
                 <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100 flex items-start gap-4 shadow-sm">
                    <ShieldCheck size={24} className="text-[#0078D4] mt-0.5 shrink-0" />
                    <p className="text-[11px] text-blue-800 font-bold leading-relaxed italic">
                      Votre compte est protégé par un cryptage de bout en bout conforme aux normes de sécurité bancaire.
                    </p>
                 </div>
           ...
           <section className="bg-white rounded-[32px] p-10 border border-[#E5E7EB] shadow-sm text-start space-y-8">
              ...
              <div className="space-y-4">
                 <SecurityAction icon={<Smartphone size={18}/>} label="Gérer les appareils de confiance" />
                 <SecurityAction icon={<Lock size={18}/>} label="Verrouiller le compte en cas de vol" color="text-amber-600" />
               ...
               <div className="lg:col-span-8 space-y-6">
                  <div className="bg-white p-10 space-y-10 border border-[#E2E8F0] rounded-[32px] shadow-sm">
                     <div className="flex items-center gap-6">
                        <div className="p-5 bg-teal-50 text-teal-600 rounded-3xl border border-teal-100 shadow-inner"><Building size={36}/></div>
                        <div>
                           <h3 className="text-2xl font-black text-[#0F172A] tracking-tight">{t.companyTitle}</h3>
                           <p className="text-sm font-medium text-[#64748B]">{t.companySub}</p>
                        </div>
                     </div>
          ...
          {activeSubTab === 'fiscality' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               <div className="bg-[#0F172A] text-white p-12 rounded-[48px] border-none shadow-2xl space-y-8 relative overflow-hidden group">
                  ...
                  <h3 className="text-2xl font-black relative z-10 text-start tracking-tight">Mises à jour automatiques</h3>
                  <p className="text-sm font-medium text-gray-400 leading-relaxed relative z-10 text-start italic">Le moteur Salery applique automatiquement les barèmes de l'IR et les taux sociaux en vigueur suite au Décret n° 2.25.983.</p>
                  <div className="p-8 bg-white/5 rounded-[32px] border border-white/10 relative z-10 text-start shadow-inner">
                     <p className="text-[10px] font-black uppercase text-teal-400 mb-2 tracking-[0.3em]">Dernière Synchronisation</p>
                     <p className="text-xl font-black">Loi de Finances 2026 v2.0 • Active</p>
                  </div>
               </div>
            </div>
          )}
