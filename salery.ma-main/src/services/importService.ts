...
export const downloadTemplate = (type: 'employees' | 'attendance') => {
  const headers = type === 'employees' 
    ? [['Employee_ID', 'First_Name', 'Last_Name', 'CIN', 'CNSS_Number', 'Position', 'Department', 'Salary_Type', 'Base_Salary', 'Overtime_Rate', 'Hiring_Date', 'Contract_Type']]
    : [['Employee_ID', 'Date', 'Check_In', 'Check_Out', 'Break_Minutes', 'Justification']];
  
  const ws = XLSX.utils.aoa_to_sheet(headers);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Salery Template");
  XLSX.writeFile(wb, `Salery_${type}_Template.xlsx`);
};
