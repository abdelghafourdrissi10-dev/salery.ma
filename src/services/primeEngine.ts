...
/**
 * Calculate seniority bonus based on prorated `gainBase`
 * (worked-days salary), NOT contractual baseSalary.
 * Unified with payrollEngine.ts logic.
 */
export const calculateSeniorityBonus = (employee: Employee, gainBase: number): number => {
  ...
  if (years >= 25) return gainBase * 0.25;
  if (years >= 20) return gainBase * 0.20;
  if (years >= 12) return gainBase * 0.15;
  if (years >= 5) return gainBase * 0.10;
  if (years >= 2) return gainBase * 0.05;
  return 0;
};