...
import {
   ...
   PlusCircle, Save, Trash, Calendar, Loader2
} from 'lucide-react';
import {
...
} from '../services/primeEngine.ts';
import { SMIG_2026 } from '../constants.ts';
...
const PrimeManager: React.FC<Props> = ({ employees, lang, user, onUpdateEmployee }) => {
   const { primes: backendPrimes, loading: primesLoading, create: createPrime, remove: deletePrime } = usePrimes();
   ...
   const t = {
      ...
      ar: {
         ...
         sub: "محرك حساب مطابق لقوانين الضمان الاجتماعي والضرائب 2026",
         btnNew: "تعويض جديد",
         btnTemplate: "إدارة القوالب",
         colEmp: "المتعاون",
         colBase: "الأجر الأساسي",
         colPrimes: "تفاصيل التعويضات",
         colRisk: "المطابقة",
         tabs: { registry: "السجل", templates: "القوالب", analytics: "التحليلات", categories: "الإعدادات" },
         risk: { low: "مطابق", high: "خطر تدقيق" },
         applyTpl: "تطبيق على الموظف",
         editTpl: "تعديل القالب",
         saveTpl: "حفظ القالب",
         addPrime: "إضافة تعويض للقالب",
         assignTitle: "تعيين تعويض",
         assignSub: "إضافة مكسب مؤقت أو متكرر لمتعاون.",
         btnConfirm: "تأكيد التعيين"
      }
   }[lang] || {
      fr: {
         title: "Gestion des Primes",
   ...
   useEffect(() => {
      // Load templates from localStorage (UI config only — not business data)
   ...
   const saveTemplates = (newTemplates: PrimeTemplate[]) => {
      ...
      // Templates are UI config — persist to localStorage only
   ...
   const handleCreateAssignment = async (e: React.FormEvent) => {
      ...
      // ✅ Persist to backend — create a company prime definition
      await createPrime({
         name: PRIME_CATEGORIES.find(c => c.id === assignmentForm.categoryId)?.name || assignmentForm.categoryId,
         amount: assignmentForm.amount,
         isSoumisCnss: PRIME_CATEGORIES.find(c => c.id === assignmentForm.categoryId)?.soumisCnss ?? false,
         isRecurring: assignmentForm.recurring,
         description: `Assigné à ${(emp as any).fullName || emp.firstName} - ${assignmentForm.categoryId}`,
      });
   ...
   const getEmployeePrimesData = (emp: Employee) => {
      const seniority = calculateSeniorityBonus(emp, emp.baseSalary);
