...
const DocumentGenerator: React.FC<Props> = ({ lang, user }) => {
   ...
   const t = {
      fr: {
         title: "Legal Engine V4",
         subtitle: "Générateur de contrats intelligents & certification QR.",
         tabDraft: "RÉDIGER", tabArchive: "ARCHIVE", tabAudit: "AUDIT",
         docType: "Type de Document", sector: "Secteur d'activité", collaborator: "Collaborateur",
         sectorServices: "Services (Tertiaire)", sectorBTP: "BTP (Secteur privé)", sectorIndustry: "Industrie / Manutention",
         selectDefault: "Sélectionner...",
         btnGenerate: "Générer Contrat V4",
         loadingText: "Salery AI Engine rédige...",
         docCertified: "DOCUMENT CERTIFIÉ", hashValidated: "HASH VALIDÉ",
         stampEmployer: "CACHET & SIGNATURE EMPLOYEUR", mentionApproval: "MENTION \"LU ET APPROUVÉ\"",
         btnCancel: "Annuler", btnArchive: "Archiver", btnDownload: "Générer & Télécharger",
         auditCenterTitle: "Legal Audit Center",
         byString: "Par"
      },
      ar: {
         title: "المحرك القانوني V4",
         subtitle: "مولد العقود الذكية ومصادقة الاستجابة السريعة (QR).",
         tabDraft: "صياغة", tabArchive: "الأرشيف", tabAudit: "التدقيق",
         docType: "نوع الوثيقة", sector: "قطاع النشاط", collaborator: "المتعاون",
         sectorServices: "الخدمات (القطاع الثالث)", sectorBTP: "البناء والأشغال العامة (القطاع الخاص)", sectorIndustry: "الصناعة / المناولة",
         selectDefault: "حدد...",
         btnGenerate: "إنشاء العقد V4",
         loadingText: "محرك Salery للذكاء الاصطناعي قيد الصياغة...",
         docCertified: "وثيقة معتمدة", hashValidated: "تم التحقق من التجزئة",
         stampEmployer: "ختم وتوقيع المشغل", mentionApproval: "عبارة \"قرئ وصودق عليه\"",
         btnCancel: "إلغاء", btnArchive: "أرشفة", btnDownload: "إنشاء وتحميل",
         auditCenterTitle: "مركز التدقيق القانوني",
         byString: "بواسطة"
      }
   }[lang] || {
      fr: {
   ...
   const validation = useMemo(() => {
      ...
      const errors = [];
      if (!company) errors.push(lang === 'fr' ? "Entreprise introuvable" : "Company not found");
      return {
      ...
      <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-7xl mx-auto px-4" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-print">
            <div className="text-start">
               <h2 className="text-4xl font-black tracking-tighter text-[#1A1F36]">{t.title}</h2>
               <p className="text-[#697386] text-sm font-medium mt-1">{t.subtitle}</p>
            </div>
            ...
            <div className="airbnb-card p-10 bg-white shadow-2xl rounded-[48px] animate-in slide-in-from-bottom-4 duration-500">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="space-y-4 text-start">
                     <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t.docType}</label>
                  ...
                  <div className="space-y-4 text-start">
                     <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t.sector}</label>
                     <select className="w-full p-6 bg-[#F7F9FC] border-2 border-transparent rounded-[32px] text-base font-bold outline-none focus:bg-white focus:border-[#0052FF]" value={sector} onChange={e => setSector(e.target.value as any)}>
                        ...
                        <option value="BTP">{t.sectorBTP}</option>
                        <option value="Industrie">{t.sectorIndustry}</option>
                     </select>
                  </div>
                  <div className="space-y-4 text-start">
                     <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t.collaborator}</label>
               ...
               {selectedEmployee && (
                  <div className="mt-12 p-8 bg-blue-50/30 rounded-[40px] border border-blue-100/50 flex justify-between items-center">
                     ...
                     <button onClick={handleGenerate} disabled={!validation?.isValid} className="px-12 py-5 bg-[#0052FF] text-white rounded-3xl font-black text-sm uppercase tracking-widest shadow-xl hover:brightness-110 active:scale-95 transition-all flex items-center gap-3">
                        <Sparkles size={20} /> {t.btnGenerate}
                     </button>
                  </div>
               )}
            </div>
         ...
         {loading && (
            <div className="py-32 flex flex-col items-center gap-8 animate-in fade-in duration-700">
               ...
               <h3 className="text-3xl font-black tracking-tighter">{t.loadingText}</h3>
            </div>
         )}
            ...
            <div className="animate-in slide-in-from-bottom-6 duration-500 pb-20 max-w-5xl mx-auto w-full">
               <div className="flex flex-col gap-8">
                  <div className="airbnb-card bg-white p-12 md:p-24 shadow-3xl relative border-none rounded-[8px] overflow-hidden">
                        ...
                        <div className="text-start space-y-4">
                           <div className="flex items-center gap-2">
                              <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-[9px] font-black uppercase border border-emerald-100 flex items-center gap-1">
                                 <CheckCircle2 size={10} /> {t.docCertified}
                              </span>
                              <span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-[9px] font-black uppercase border border-blue-100 flex items-center gap-1">
                                 <Fingerprint size={10} /> {t.hashValidated}
                              </span>
                           </div>
                     ...
                     <div className="mt-32 pt-16 border-t border-gray-100 grid grid-cols-2 gap-16">
                        <div className="text-start space-y-6">
                           <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">{t.stampEmployer}</p>
                        ...
                        <div className="text-right flex flex-col justify-between items-end">
                           <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">{t.mentionApproval}</p>
                           <div className="pt-20">
                  ...
                  <div className="flex gap-4 no-print">
                     <button onClick={() => setRawOutput(null)} className="flex-1 py-6 bg-rose-50 text-rose-600 border border-rose-100 rounded-3xl font-black text-xs uppercase tracking-widest shadow-sm hover:bg-rose-100 transition-all">{t.btnCancel}</button>
                     <button onClick={() => handleSaveDocument('draft')} className="flex-1 py-6 bg-blue-50 text-[#0052FF] rounded-3xl font-black text-xs uppercase tracking-widest border border-blue-100 hover:bg-blue-100 transition-all">{t.btnArchive}</button>
                     <button onClick={() => handleSaveDocument('exported')} className="flex-1 py-6 bg-[#0052FF] text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-200 hover:brightness-110 transition-all">{t.btnDownload}</button>
                  </div>
               </div>
            </div>
         )}
         ...
         {activeTab === 'audit' && (
            <div className="airbnb-card bg-white p-10 shadow-2xl rounded-[48px] text-start animate-in slide-in-from-bottom-4">
               <h3 className="text-2xl font-black text-[#1A1F36] mb-8 flex items-center gap-3"><Shield className="text-[#0052FF]" /> {t.auditCenterTitle}</h3>
                     ...
                     <div key={i} className="flex items-center justify-between p-6 bg-[#F7F9FC] rounded-[32px] border border-transparent hover:border-blue-100 transition-all">
                        <div className="flex items-center gap-5">
                           ...
                           <div>
                              ...
                              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t.byString} {log.user} • {log.time}</p>
                           </div>
                        </div>
