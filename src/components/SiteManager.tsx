...
import {
  ...
  BarChart3, Users, Clock, ShieldCheck, Zap, X, Save,
...
const SiteManager: React.FC<Props> = ({ employees, lang, user }) => {
  ...
  const t = {
    fr: {
      title: "Contrôle Souverain des Sites",
  ...
  const handleOpenEdit = () => {
    ...
    setFormData({
      ...
      budget: selectedSite.budget,
      workTemplateId: selectedSite.workTemplateId || 'TPL-44H-6D',
      autoPrimes: true,
    });
    setEditMode(true);
  };
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    const updated: Site = {
      ...
      workTemplateId: formData.workTemplateId,
    };
  ...
  const handleSuspend = () => {
    if (!selectedSite) return;
    const newStatus = selectedSite.status === 'active' ? 'suspended' : 'active';
    const updated = { ...selectedSite, status: newStatus as Site['status'] };
    setSites(prev => prev.map(s => s.id === updated.id ? updated : s));
    setSelectedSite(updated);
  ...
  const handleTogglePrime = (siteId: string, categoryId: string, currentStatus: boolean) => {
    if (!['SUPER_ADMIN', 'COMPANY_OWNER', 'DIRECTEUR_RH', 'TEAM_RH', 'RH_ADMIN'].includes(user.role)) return;
  ...
  if (selectedSite) {
    return (
      <div className="flex flex-col bg-[#F9FAFB] border border-[#E5E7EB] rounded-[24px] min-h-[calc(100vh-120px)] animate-in slide-in-from-right duration-500 text-start overflow-hidden shadow-sm relative">
        {/* COMPACT SITE HEADER */}
        <div className="bg-white border-b border-[#E5E7EB] p-6 md:px-10 shrink-0 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex gap-2 shrink-0">
              <button
                ...
                className="px-4 py-2 bg-[#F9FAFB] border border-[#E5E7EB] text-gray-500 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-white hover:border-[#0078D4] hover:text-[#0078D4] transition-all shadow-sm"
              >
                <Edit3 size={14} /> MODIFIER
              </button>
              <button
                onClick={() => setConfirmSuspend(true)}
                className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-sm border ${selectedSite.status === 'active'
                  ? 'bg-rose-50 text-rose-600 border-rose-100 hover:bg-rose-100'
                  : 'bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-100'
                  }`}
              >
                <Power size={14} /> {selectedSite.status === 'active' ? 'SUSPENDRE' : 'RÉACTIVER'}
              </button>
            </div>
          </div>
        ...
        <div className="p-6 md:p-10 flex-1 relative z-0">
        ...
        {/* STICKY SAVE BUTTON */}
        <div className="bg-white border-t border-[#E5E7EB] p-6 flex justify-end gap-4 shrink-0 mt-auto sticky bottom-0 z-20">
          <button
        ...
        {confirmSuspend && (
          <div className="fixed inset-0 z-[3000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-white p-10 rounded-[40px] shadow-3xl max-w-sm text-center space-y-6 border border-rose-100 animate-in zoom-in duration-300">
              <div className={`w-16 h-16 rounded-3xl flex items-center justify-center mx-auto shadow-sm ${selectedSite.status === 'active' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'
                }`}>
                <Power size={32} />
              </div>
              <h3 className="text-xl font-black text-[#1A1F36]">
                {selectedSite.status === 'active' ? 'Suspendre ce site ?' : 'Réactiver ce site ?'}
              </h3>
              <p className="text-xs text-gray-400 font-bold leading-relaxed">
                {selectedSite.status === 'active'
                  ? `Le site "${selectedSite.name}" sera suspendu. Le pointage et les primes seront désactivés jusqu'à réactivation.`
                  : `Le site "${selectedSite.name}" sera remis en service.`
                }
              </p>
              <div className="flex gap-3">
                <button onClick={() => setConfirmSuspend(false)} className="flex-1 py-3.5 bg-gray-50 text-gray-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-gray-100 transition-all">ANNULER</button>
                <button
                  onClick={handleSuspend}
                  className={`flex-1 py-3.5 rounded-2xl font-black text-[10px] text-white uppercase tracking-widest shadow-lg transition-all ${selectedSite.status === 'active' ? 'bg-rose-600 shadow-rose-200 hover:bg-rose-700' : 'bg-emerald-600 shadow-emerald-200 hover:bg-emerald-700'
                    }`}
                >
                  {selectedSite.status === 'active' ? 'SUSPENDRE' : 'RÉACTIVER'}
                </button>
              </div>
            </div>
          </div>
        ...
        {/* ── Edit modal ──────────────────────────────────────── */}
        {editMode && (
          <div className="fixed inset-0 z-[3000] bg-black/70 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-xl rounded-[40px] shadow-3xl overflow-hidden animate-in zoom-in duration-300 border border-gray-100">
              <header className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-blue-50 text-[#0078D4] rounded-xl flex items-center justify-center">
                    <Edit3 size={20} />
                  </div>
                  <div>
                    <h3 className="text-lg font-black text-[#1A1F36] tracking-tight leading-none">Modifier le Site</h3>
                    <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest mt-1">{selectedSite.code}</p>
                  </div>
                </div>
                <button onClick={() => setEditMode(false)} className="p-2 bg-white hover:bg-gray-100 rounded-full text-gray-400 transition-all shadow-sm"><X size={20} /></button>
              </header>
              <form onSubmit={handleSaveEdit} className="p-8 space-y-5 text-start">
                <div className="grid grid-cols-2 gap-5">
                  <FormInputCompact label="Nom du Site" value={formData.name} onChange={(v: string) => setFormData({ ...formData, name: v })} required placeholder={selectedSite.name} />
                  <FormSelectCompact
                ...
                <div className="grid grid-cols-2 gap-5">
                  <FormInputCompact label="Ville" value={formData.city} onChange={(v: string) => setFormData({ ...formData, city: v })} required placeholder={selectedSite.city} />
                  <FormInputCompact label="Budget (DH)" type="number" value={formData.budget} onChange={(v: string) => setFormData({ ...formData, budget: Number(v) })} placeholder={String(selectedSite.budget)} />
                </div>
                <FormInputCompact label="Adresse" value={formData.address} onChange={(v: string) => setFormData({ ...formData, address: v })} placeholder={selectedSite.address} />
                <FormSelectCompact
                  label="Gabarit Temps"
